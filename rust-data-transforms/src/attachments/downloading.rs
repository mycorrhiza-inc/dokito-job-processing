...
impl DownloadIncomplete for ProcessedGenericAttachment {
    ...
    async fn download_incomplete(
        ...
        // Check Redis cache using new system
        if let Some(cached_attach) = lookup_attachment_from_redis(&self.url).await {
        ...
        // Cache the attachment using new Redis system
        if let Err(err) = cache_attachment_in_redis(&final_raw_attachment, extra_data.fixed_jurisdiction).await {
            warn!(%err, url = %self.url, %hash, "Failed to cache attachment in new Redis system");
        }
